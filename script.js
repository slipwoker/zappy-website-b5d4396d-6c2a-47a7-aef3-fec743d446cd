document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            var emailFields = contactForm.querySelectorAll('input[type="email"]');
            for (var m = 0; m < emailFields.length; m++) {
                var emailField = emailFields[m];
            ...
            if (valid) {
                // Submit form programmatically after validation
                contactForm.submit();
            } else if (firstError) {
                firstError.focus();
            }
        ...
        contactForm.addEventListener('input', function(e) {
            ...
            if (!field || !field.hasAttribute('required') && field.type !== 'email') return;
            var errorEl = field.parentNode.querySelector('.form-error');
            var value = field.value.trim();
            if (field.hasAttribute('required') && !value) {
                if (!errorEl) showError(field, 'This field is required');
            } else if (field.type === 'email' && value && !isValidEmail(value)) {
                if (!errorEl) showError(field, 'Please enter a valid email address');
            } else {
    ...
    if (animatedElements.length > 0) {
        ...
        for (var n = 0; n < animatedElements.length; n++) {
            animatedElements[n].classList.add('fade-in-hidden');
            observer.observe(animatedElements[n]);
        }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b5d4396d-6c2a-47a7-aef3-fec743d446cd',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b5d4396d-6c2a-47a7-aef3-fec743d446cd';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b5d4396d-6c2a-47a7-aef3-fec743d446cd';
